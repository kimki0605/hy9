...
/**
 * httpServletReqeust 클래스 메소드 확인
 * http://localhost:8080/xxx/login 요청을 처리하는 서블릿 
 */
...
public class LoginServlet extends HttpServlet {
	 ...
	 * HttpServletRequest객체 : 클라이언트 요청시 발생된 모든 정보와 요청할 때 같이 전달된 클라이언트 입력데이터를 갖고 있음 
	 * 모든 네트워크는 1byte로 데이터로 구성 후 전송됨 
	 * 데이터중 결합데이터는 네트워크로 전송시 분리되므로 사용전에 결합해야 함
	 * 결할할때 encoding 방식을 전달해야 함 ->HttpServletRequest.serCharacterEncoding("utf-8")
	 */
