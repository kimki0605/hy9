 ...
 * 작은 /pivot/ 큰
 * 비교하면서 데이터를 찾기 때문이 비교정렬
 * 정렬 대상 데이터외에 추가적인 공간을 필요로 하지 않는다 : in-palce sort
 * 
 * 1.pivot 선택
 * 2.pivot 기준 - 왼쪽에서부터는 피벗보다 큰 값을 오른쪽에서부터 피벗보다 작은 값을 찾는다
 * 3.양방향에서 찾아지면 교환한다
 * 4.양쪽 위치 idx가 서로 교차되지 않을때까지 2,3반 빈벅
 * 5.교차지좀을 기준으로 두 개의 부분리스트로 나누고 각 부분리스트에 퀵정렬 과정(1~4)을 반복(Divide)
...
public class QuickSort {
	 ...
	 * 왼쪽 피벗 선택 방식
	 * @param a : 정렬할 배열
	 * @param lo : 현재 부분 리스트 왼쪽
	 * @param hi : 현재 부분 리스트 오른쪽 원소 인덱스
	 */
	private static void l_pivot_sort(int[] a, int lo, int hi) {
		//재귀 호출의 종료 조건 : 피벗 기준으로 왼쪽 오른쪽 부분리스트가 없거나
		if(lo >= hi) { //정렬할 원소가 없거나 1개
			return;
		}
		
		//두개의 리스트로 분할 후 pivot을 반환
		int pivot = partiton(a, lo, hi); //pivot 위치 index
	 ...
	 */
	private static int partiton(int[] a, int left, int right) {
		int lo = left;
		int hi = right;
		...
		while(lo<hi) {
			 ...
			 * hi를 감소
			 */
			while(a[hi]>pivot && lo < hi) { //height 포인터가 피벗보다 작은값을 찾은게 아니라면 현재 원소는 
				lo++;
			}
