...
public class StudentController {
	...
	@RequestMapping("/student/studentDetailView/{stdNo}/{stdName}/{stdYear}") //stdNo pathVariable과
	...
	public String studentSearch(@RequestParam HashMap<String, Object> param) {
		...
		System.out.println(param.et("type"));
		System.out.println(param.et("keyword"));
	...
	//form 태그 내 input의 name 속성값이 매개변수와 동일하면 @ReauestParam을 생략할 수 있다
	//파라미터명과 매개변수 명이 동일 한 경우
	@RequestMapping("/student/newStudent3")
	public String insertStudent3(@RequestParam("no")String no,
								 @RequestParam("name")String name,
								 @RequestParam("year")String year,
								 Model model) {
		model.addAttribute("no",no);
	...
	//command 객체 사용 시 주의사항 : 파라미터명과 필드명이 동일해야 함 다르면 매칭되지 않는다(null이 저장됨)
